...
class Block(nn.Module):
    ...
    def forward(self, x, knn):
        ...
        for i in range(self.depth):
            x = x + self.drop_paths[i](self.dfils[i](x, knn))
            if i % 2 == 1:
...
class Stage(nn.Module):
    ...
    def forward(self, x, xyz, prev_knn, pwd):
        ...
        if not self.first:
            ...
            B, N, C = x.shape
            x = self.skip_proj(x.view(B * N, C)).view(B, N, -1)[:, :self.n] + self.dfil(x, prev_knn)[:, :self.n]
...
class LANetCls(nn.Module):
    ...
    def forward(self, xyz):
        if not self.training:
            ...
            # resample
            idx = pointnet2_utils.furthest_point_sample(xyz, 1200).long()[:,
                  torch.randperm(1200, device=xyz.device)[:1024]]
